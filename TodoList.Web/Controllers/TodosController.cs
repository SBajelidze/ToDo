...
namespace TodoList.Web.Controllers
{
    ...
    public class TodosController : Controller
    {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> EmptyCompletedTodos()
        {
            var currentUser = await _userManager.GetUserAsync(User);
            if (currentUser == null) return Challenge();

            var successful = await _todoItemService.EmptyCompletedTodosAsync(currentUser);

            if (!successful)
            {
                return BadRequest("Could not empty completed todos.");
            }
        ...
        public async Task<IActionResult> Copy(Guid id)
        {
            ...
            var todoToCopy = await _todoItemService.GetItemAsync(id);
            ...
            // Create a new TodoItemCreateViewModel and populate it with the copied item's properties
            var copiedTodoViewModel = new TodoItemCreateViewModel
            {
                Title = todoToCopy.Title,
                Content = todoToCopy.Content,
                DuetoDateTime = todoToCopy.DueTo.ToDateTimeUtc(),
                Tags = todoToCopy.Tags != null ? string.Join(',', todoToCopy.Tags) : ""
            };

            return View("Create", copiedTodoViewModel);
        }
